...
4. Implement Details artwork policy: `Backdrop`, then `Thumb`, then `Banner`, then cropped `Primary`, then black/matte no-art fallback.
5. Implement Playback OSD using the compact strip model. Keep OSD internal padding; do not let focused controls touch the panel edge.
6. Use real Emby artwork only in temporary/local visual tests. Do not commit private server URLs, credentials, or private media images.
