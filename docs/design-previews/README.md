...
Ideal native render targets:

- `A3-ideal-home-dashboard.png`: target mood for Home density, source-aware left Guide, rails, and artwork-led color.
...
These generated renders express the desired native feel, especially material, focus priority, and artwork atmosphere. They are not implementation screenshots and must not override `DESIGN.md` token limits, fallback rules, or accessibility requirements.
