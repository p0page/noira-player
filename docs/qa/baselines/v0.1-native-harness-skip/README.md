...
```powershell
dotnet run --project tools\NextGenEmby.PlaybackQuality.Cli\NextGenEmby.PlaybackQuality.Cli.csproj -- materialize-native-harness-report-set --manifest docs\qa\playback-quality-reference-manifest.example.json --reports-dir docs\qa\baselines\v0.1-native-harness-skip\reports --source-revision working-tree-track-external-default-forced --player-core-version NextGenEmby.Core --build-configuration Debug --output docs\qa\baselines\v0.1-native-harness-skip\materialized-native-harness-summary.json
