...
| native/App 采集入口存在 | 部分满足 | DEBUG App `quality-run` 路径和 `Export-AppQualityRunReports.ps1` | App-hosted collector 可以写 LocalFolder 报告，但目前没有已提交的公开 native/App captured baseline。 |
...
1. 使用现有 reference manifest 和公开 sample case。
2. 运行 App-hosted DEBUG `quality-run`，或实现等价的最小 native/App 软件 harness。
