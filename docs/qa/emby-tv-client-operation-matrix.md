...
| Seek preview and cancel | `Shift+Left/Right`, `Escape`, `Return` | Implemented | 0.1.0.143 added core keyboard surrogate policy and prompt tests for `Shift+Left/Right`, `A/Enter Confirm`, and `B/Escape Cancel`; app build installed locally. Computer Use could not complete a fresh seek-preview run because the local install no longer had a saved Emby session and manual direct-stream playback did not form a seekable playback context | Restore a playable saved session or add a reliable seekable local QA fixture, then validate preview cancel/confirm with keyboard only |
