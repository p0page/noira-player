...
| Browse Playlists root | DEBUG route `playlists-fixture`, arrows | Verified | 0.1.0.185 opened `Playlists` with two `Playlist` cards, packaged wide artwork, and focus on `Weekend Queue` | Re-run against real server `Playlist` rows after session restore |
| Open Playlist contents and return | `Return`, `Right`, `Right`, `Escape` | Verified | 0.1.0.185 opened `Weekend Queue` as a nested Library with 5 mixed child items; two `Right` presses moved focus to `Ocean Archive`; long episode labels stayed clipped within cards; `Escape` returned focus to `Weekend Queue` | Add real-server playlist child validation; if `ParentId` is insufficient for some Emby servers, add the official playlist-items endpoint while preserving compatibility |
...
1. Collections and Playlists now have deterministic positive fixture validation in 0.1.0.185, including root cards, nested child browsing, horizontal focus movement, and B/Escape focus restoration. Remaining risk is real-server child loading, especially playlist servers that require a dedicated playlist-items endpoint instead of `ParentId`.
2. Details fixture coverage is now broad: default focus, version selection, favorite/watched toggles, similar rail, cast rail, and add-to sheets are keyboard-verified in 0.1.0.175/0.1.0.166. Remaining Details risk is live server add-to success on disposable user-data, collection, and playlist targets, plus a real similar-items response after session restore.
