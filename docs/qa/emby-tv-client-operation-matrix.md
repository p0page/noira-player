...
| Seek preview and cancel | `Shift+Left/Right`, `Escape`, `Return` | Implemented | 0.1.0.143 added core keyboard surrogate policy and prompt tests for `Shift+Left/Right`, `A/Enter Confirm`, and `B/Escape Cancel`; 0.1.0.144 added a DEBUG manual-playback dev-command fixture that accepts `streamUrl` and `autoStart`, installed locally, prefilled the manual URL textbox, and opened a public MP4 into native `Playing` state. Computer Use could not complete a fresh seek-preview run because the local install no longer had a saved Emby session and the available short public MP4 ended before a full preview cancel/confirm route could be completed | Use the prefilled manual fixture with a longer seekable URL or restored saved session, then validate preview cancel/confirm with keyboard only |
...
2. Playback stream switching and seek-preview cancel still need fresh keyboard validation on real multi-audio/subtitle-rich media; the 0.1.0.138 pass verified transport focus activation, pause/resume, seek-forward button activation, and More open/close, while 0.1.0.143 added the seek-preview keyboard surrogate and fixed handled `Escape` closing for the More drawer.
