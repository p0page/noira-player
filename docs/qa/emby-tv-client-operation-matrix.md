...
1. Details fixture coverage is now broad: default focus, version selection, similar rail, cast rail, and add-to sheets are keyboard-verified in 0.1.0.166. Remaining Details risk is live server add-to success on a disposable collection/playlist target and a real similar-items response after session restore.
