...
| Server-configured home sections | Rail navigation | Verified | 0.1.0.156 fixed DEBUG fixture library/section artwork so Hot Movies, Hot TV Series, Douban Top Rated, Netflix, Anime, and Documentaries render dedicated wide cards while the saved session is absent; 0.1.0.132 validated Home row `More` for Hot Movies; 0.1.0.102 parsed many server categories with section artwork | Re-run screenshots against real server artwork after session restore |
...
| Move scopes | `Down`, `Right` to far edge, `Return`, `Down`, `Up`, `Left` | Verified | 0.1.0.161 DEBUG `search-fixture` validated the full Search scope rail with fixture results: `Down` to All, nine `Right` presses to far-right `Live TV` while keeping it visible, `Return` filtered to `1 result / Live TV`, `Down` focused `News 24`, `Up` returned to `Live TV`, and `Left` moved to `Photos`; 0.1.0.129 `Down`, `Down`, `Return` moved from query toward results and opened the first result; `SearchFocusNavigationPolicy` tests cover directional decisions | Keep as Search scope regression route |
