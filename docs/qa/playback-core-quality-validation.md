...
`evidenceGates` 是模型优先读取的门禁摘要。它按顺序列出 `manifest`、`baseline-report-set`、`candidate-report-set`、`candidate-report-analysis`、`suite`，每一项都有 `status`、`action`、`blockers`、`signals` 和 `caseIds`。当 report-set gate 被阻断时，模型应优先根据 `signals` 和 `caseIds` 修复采集或源选择；当 `candidate-report-analysis` 被阻断时，模型应先处理 candidate `modelAnalysis.optimizationGate` 给出的 blocker，例如 source mismatch、缺失证据或样本不足；当 suite gate 为 `skipped` 时，说明还没有进入播放核心 before/after 比较。
