...
`tools/NextGenEmby.PlaybackQuality.Cli` 是 App-free 的命令行入口，用来从已序列化的报告文件生成模型可消费的分析和比较 JSON。`analyze-report` 命令要求传入 `--report`，输入可以是 raw `PlaybackQualityReport`，也可以是顶层包含 `report` 字段的 `PlaybackQualityRunResult` envelope；命令会重新运行 `PlaybackQualityReportAnalyzer`，并把 `PlaybackQualityModelAnalysis` 写到 stdout 或 `--output`。当自动化模型只需要诊断一次采集结果、判断是补证据还是修改播放 Core 时，应优先使用这个入口。`analyze-report` 的 `missingEvidence` 与 reference required-signal 契约对齐：pin 住颜色输出时会标记缺失的 `display.hdrStatus`、swapchain 格式/色彩空间和 HDR10 十位 swapchain 证据。`analyze-report-set` 命令要求传入 `--reports-dir`，会分析目录下所有 raw report 或 envelope；如果 envelope 里的 `modelAnalysis.runId` 或 `modelAnalysis.result` 缺失，或者 `modelAnalysis.analyzerVersion` 不等于当前 `PlaybackQualityReportAnalyzer.CurrentAnalyzerVersion`，该 analysis 必须被视为不可复用并重新生成。命令输出与候选评测共用的 report-analysis summary；summary 必须输出 `schemaVersion = 1`、`action`、`risk` 和 `confidence`，并聚合 `blockers`、optimization gate signals、analyzer evidence signals、`failureAreas`、最高优先级 `targetFailureAreas`、对应 `targetCaseIds` 和 `codeTargets`，每个 case 也必须保留 `blockers`、`signals`、`failureAreas`、`targetFailureAreas` 和 `codeTargets`。模型应先读取 `action`、`risk` 和 `confidence.level` 判断能否优化 Core，再用 `blockers`、`targetFailureAreas`、`targetCaseIds` 和 `codeTargets` 定位样本与可能需要查看的 Core/native 文件。`compare` 命令要求传入 `--baseline` 和 `--candidate`，支持重复传入 `--previous` 做停滞检测，支持 `--stall-threshold`，并输出到 stdout 或 `--output`。这些输入同样可以是 raw `PlaybackQualityReport` 或包含顶层 `report` 的 `PlaybackQualityRunResult` envelope。CLI 是连接已采集 quality-run artifact 和自动化模型优化循环的首选桥接方式，因为它不需要临时自定义代码，也不会构建或打包 Xbox App。
