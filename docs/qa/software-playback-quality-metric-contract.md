...
`environment.sourceRevision` and `environment.playerCoreVersion` are build identity evidence for automated before/after loops. They do not prove playback quality by themselves, but they let a model tie a candidate report back to the Core revision that produced it. If these fields are absent, comparisons can still run, but the optimization loop has weaker provenance and should prefer recollecting reports with build identity before accepting or rejecting a Core change.
...
Suite 还会输出 `environment` 摘要，聚合 `missingEvidenceCount`、`partialCount`、`sameBuildCount`、`differentBuildCount`、对应 case id 列表和环境信号。模型在读取 suite 总览时必须先检查这些字段：`sameBuildCount > 0` 表示 baseline 和 candidate 没有证明来自不同 Core 构建，不能把表面上的 improved 结果当成可接受优化，只能进入 `collect-comparable-evidence`。
