...
`PlaybackQualityReportMapper.ApplyDisplayStatus` is the canonical Core mapping from `PlaybackDisplayStatus` into `display` and `colorPipeline`. `PlaybackQualityReportMapper.ApplyMetrics` is the canonical mapping from playback metrics snapshots into `timing`, `sync`, and `buffers`. App or harness code should use these mappers instead of copying report fields manually.
