...
`position.requestedStartPositionTicks`, `position.seekTargetPositionTicks`, `position.actualPositionTicks`, and `position.seekPositionErrorMs` describe software-observed timeline behavior after resume or seek operations. When `expected.maxSeekPositionErrorMs` is supplied, the evaluator uses `position.seekPositionErrorMs` if present, otherwise derives the absolute error from seek target and actual position ticks. Exceeding the threshold or missing the required position evidence is a `timeline` failure. The model should inspect seek preview commit/cancel state, resume/start position propagation, demux seek completion, and native position reporting before changing frame pacing or A/V sync logic.
...
```powershell
dotnet run --project tools\NextGenEmby.PlaybackQuality.Cli\NextGenEmby.PlaybackQuality.Cli.csproj -- materialize-run-result --report raw-report.json --output run-result.json
```
