...
- 如果 `activeGate.name = suite` 且被阻断，优先读取 `activeGate.environment`、`activeGate.confidence`、`activeGate.resultCounts`、`activeGate.targetFailureAreas` 和 `activeGate.targetCaseIds`，再按需展开 `suite.cases` 或单 case comparison。
