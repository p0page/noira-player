...
`PlaybackQualityReferenceReportSetValidator` 会在 baseline/candidate 比较前校验已采集报告是否覆盖 reference manifest。它按 `report.runId` 匹配 manifest `caseId`，并对缺失报告、额外报告、重复 runId、source metadata 不匹配以及 case 级 `requiredSignals` 缺失输出结构化失败。缺失 telemetry 会记录为 `report.requiredSignal.missing`，并带上精确的 `signal` 和 `caseId`，让自动化循环先修复采集或源选择，再比较 Core 改动。CLI 从 JSON 读取 report-set 时会保留字段 presence，因此 `videoStarvedPasses: 0` / `audioStarvedPasses: 0` 这类显式 0 counter 会被视为已采集，而字段缺失才会被视为 missing evidence。manifest 提供可选 HDR/Dolby Vision source strategy 字段时也会一并校验。自动化循环必须把无效 report set 视为证据采集失败，而不是播放 Core 回归或改进。
