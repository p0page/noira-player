...
The suite also emits `cases`, a compact per-comparison summary containing `caseId`, run IDs, result, decision, action, risk, confidence, `suggestedNextAction`, reasons, signals, failure areas, and blockers. `failureAreas` must include resolved, new, and persisting failure areas; unchanged comparisons with persisting failures still surface their matched signals so a model can continue optimizing the correct playback Core area. `suggestedNextAction` and `reasons` must preserve the case-level action rationale so automated consumers can localize the sample and understand the next operation before expanding the full `comparisons` payload.
