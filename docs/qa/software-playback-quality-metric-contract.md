...
`source.container` and `source.bitrate` preserve Emby media-source metadata when it is already available in `PlaybackDescriptor.MediaSource`. They are diagnostic evidence for source selection, protocol, and high-bitrate playback triage; they are not currently stable pass/fail thresholds. Duration and chapter evidence are not yet present in the playback-quality descriptor path and must not be inferred from filenames or sample defaults.
