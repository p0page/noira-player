...
In App/native playback sessions, the WinRT adapter is expected to provide runtime metrics by mapping `NativePlaybackEngine.QualityMetrics()` into `PlaybackQualityMetricsSnapshot` through `IPlaybackQualityMetricsProvider`. This proves only that software counters reached the Core report path; it does not prove HDMI/display output correctness or replace an actual playback harness run.
