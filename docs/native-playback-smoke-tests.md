...
- 当前 native 解码器已能打开 FFmpeg format/codec context，能读 packet / 接收 `AVFrame` 元数据，并会在 codec 支持时尝试使用 D3D11VA hardware device context；但还不会输出可呈现的 D3D11 texture slice。
