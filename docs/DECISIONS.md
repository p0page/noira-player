...
# 2026-07-15: 双 EAGAIN 恢复必须成为正式播放证据

决策：evaluation version 升级为 `playback-quality-v0.17`。所有已打开 native 视频解码器的正式播放报告必须显式记录 `videoDecoderSendPacketEagainCount`、`videoDecoderDoubleEagainRetryCount`、`videoDecoderDoubleEagainRecoveryCount` 和 `videoDecoderDoubleEagainExhaustedCount`。缺失字段不能按零处理；非 native 报告的默认零值不能生成“恢复未耗尽”的 pass。计数内部矛盾属于 `evaluation harness bug`，有界恢复耗尽属于 `decoder-recovery / player-core bug`。

原因：仅测试恢复状态机无法证明真实 FFmpeg/D3D11 播放是否进入该状态。私有代表片源已经自然观察到 `2/2/2/0` 与 `1/1/1/0`，证明同一压缩包的有界重试在真实 native 路径发生并恢复。该证据只支持当前恢复链路有效，不支持放宽预算、隐藏耗尽或宣称所有 HEVC 输入均已稳定。本轮不改变现有解码策略。

决策：评测 artifacts 的并发和文件发现属于裁判契约。共享 native-headless artifacts 目录必须由 worktree 级进程 mutex 串行化；report-set reader 只把 playback report 或 run-result envelope 当报告，同目录 runner summary、validation 和 analysis JSON 必须忽略。合法但 malformed 的播放报告仍必须进入校验并失败，不能因过滤辅助文件而被静默跳过。

原因：并发 smoke 曾互相删除同一目录并产生跨 case 污染；manifest runner 默认也允许 summary 与 reports 同目录。两者都会让评测器制造与播放器无关的假失败。合法但无法识别的 JSON 必须在发现阶段失败，不能被静默跳过；修复后仍保留 duplicate run-id、extra report、missing report 和 required signal 的严格检查。

# 2026-07-15: report 必须证明属于当前 native invocation

决策：manifest runner 在调用 native harness 前为每个 case 生成唯一 `attemptId`，并在执行前删除该 case 的旧报告和 helper transcript。headless 只能把 runner 提供的 identity 写入本轮报告；runner 必须精确比较预期和实际 `execution.attemptId`。物理文件存在但 identity 缺失或不同，一律记为 `invalid-report` 和 unattributed，结果不得沿用报告里的 pass/fail。

baseline 必须在 materialize 前检查 runner summary 的版本、缺失报告数、invalid report 数和 unattributed report 数；任一非零都终止归档。匹配当前 attempt 的真实 player fail/error 可以继续进入严格报告校验，因为播放失败也是有效证据；来源不明的内容则不是播放证据，无论其 JSON 结构和指标多完整都不能进入 baseline。

本决策将运行编排合同升级为 `native-manifest-runner-v0.2`，但不改变播放报告 schema 和 v0.17 评价语义，因此不升级 `playback-quality-v0.17`。若后续引入跨进程重试、分布式执行或一个 attempt 多份报告，必须先定义新的身份和父子关系合同，不能通过放宽 attempt 精确匹配来兼容。

# 2026-07-15: 启动超时必须先区分传输等待与 Core 处理

决策：evaluation version 升级为 `playback-quality-v0.18`。当 `startupDurationMs` 超过 case 门限时，裁判必须汇总各启动阶段中 `transportCallEvidenceStatus=measured` 的 read/seek wait。只有实测等待不小于超时量，且至少占总启动时长 25%，才把该失败归为 `transport-wait-dominant / test sample or environment issue`，并禁止模型仅凭本轮结果调整 Core open/demux 策略。等待不足以解释超时时，仍归为 `startup-processing-dominant / player-core bug`；缺少实测传输证据时不得猜测。启动未超限时只标记 `within-threshold`，不作因果归因。
