...
$nativeHttpMediaInputCommand = '"' + $vcvars + '" >nul && if not exist C:\tmp\noiraplayer-http-media-input mkdir C:\tmp\noiraplayer-http-media-input && cl /nologo /std:c++20 /EHsc /utf-8 /DWIN32_LEAN_AND_MEAN /DWINRT_LEAN_AND_MEAN /I src\NoiraPlayer.Native /I src\NoiraPlayer.Native\packages\FFmpegInteropX.UWP.FFmpeg.8.1.2\include /Fo:C:\tmp\noiraplayer-http-media-input\ tests\NoiraPlayer.Native.Tests\HttpMediaInputTests.cpp src\NoiraPlayer.Native\Media\HttpMediaInput.cpp /Fe:C:\tmp\HttpMediaInputTests.exe /link /LIBPATH:src\NoiraPlayer.Native\packages\FFmpegInteropX.UWP.FFmpeg.8.1.2\runtimes\win-x64\native avformat.lib avcodec.lib avutil.lib windowsapp.lib && set PATH=%CD%\src\NoiraPlayer.Native\packages\FFmpegInteropX.UWP.FFmpeg.8.1.2\runtimes\win-x64\native;%PATH% && C:\tmp\HttpMediaInputTests.exe'
...
$commands = @(
    ...
    New-CommandPlan `
        ...
        -Command 'cmd' `
        -Arguments @('/c', $nativeHttpMediaInputCommand)
    New-CommandPlan `
